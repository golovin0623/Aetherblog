...
浮岛的三个密度(灵动音乐元 → 迷你播放器 → 沉浸播放台)此前只是「功能上能切换」,四处交接各自缺了动效语法。本轮逐条补齐,全部改动限定在窄屏(`@media (max-width: 768px)` / `isMobile`)。指针端的形变时序(实测 `--music-morph-dur` 仍为 `520ms` / `--ease-out`,内容延迟 `0ms`)与出入场 variants 均与本轮之前逐字一致;唯一例外是 `prefers-reduced-motion` 那条两端共用的路径,时长由 120ms 收到 80ms(对齐音乐域已在用的 `musicMotion.duration.reduced`)。
...
- **令牌层**:`packages/ui/src/motion.ts` 的 `musicMotion` 新增 `ease.emphasis/recede`、`spring.islandEnter/sheetZoom`、`duration.islandEnter/islandExit/morph/contentIn/contentOut/contentDelay`、`island.*` 缩放档;`music-skin.css` 新增 `--music-morph-{dur,ease}` / `--music-content-{dur,delay}` / `--music-ease-emphasis` / `--music-ambient-blur`,默认值即桌面既有行为(`--music-ease-recede` / `--music-content-out-dur` 在第二评审轮随桌面时序还原一并删除,recede 只余 Framer 侧消费)。删除随 `layoutId` 一起失效的 `spring.sheet` 与 `duration.zoom`。
- **PR #865 评审轮(Codex 两条 P2,复核均成立)**:① **指针端被移动端改动波及** —— 锚角缩放 variants 原本只按 `isMobile` 切换缩放幅度,指针端仍吃到了 `spring.islandEnter` + 340ms 透明度,替掉了既有的 opacity-only / `transition.quick`。这同时暴露出初版描述「桌面时序逐字节不变」的失准:当时只核对了 CSS 形变时序,漏了无条件生效的 Framer 出入场。现新增 `pointer*` 一组 variant(与本轮之前逐字等价),由 `resolveMusicIslandMotion(isMobile, prefersReducedMotion)` 按视口/无障碍偏好择一;`musicMotion.island` 的 `enterScale`/`exitScale` 双档收敛为标量,`custom` 简化为布尔 `handoff`(交接本就只在触屏成立 —— 指针端沉浸态仍在同一壳体内,壳体不会卸载)。② **`sheetOrigin` 跨会话残留** —— 它只在 `openImmersivePlayer` 里赋值且从不清除,而 `MusicHallExperience:302`(`onOpenDeck`)、`ProfileMusicPlayer:338`、`MusicPlayerProvider:1020`(`options?.expand`)三处都是直接 `setExpanded(true)`,于是「从浮岛展开过一次 → 收起 → 改从那两个入口打开」时,台面会从一个与本次触发无关的左下角旧坐标窜出来。**清空时机是关键**:不能在 `expanded` 转 `false` 时清,那会让正在跑的回收动画中途丢掉 `transform-origin`,恰好破坏本轮要建立的连续性;改挂沉浸台的 `onAnimationComplete`,退场跑完再清,且读 `expandedRef` 而非闭包里的 `expanded`(退场渲染的是 `AnimatePresence` 缓存的那一帧,其闭包里 `expanded` 仍为 `true`)。另 `openImmersivePlayer` 在测不到浮岛矩形时显式置 `null`。失败方向安全:回调万一不触发,只退化成「原点不清」的现状,不破坏动画。**未做**:大厅正在播放条 / Profile 卡的「自触发点原位放大」—— 需要它们经 context 上报各自 rect,是另一尺度的改动,本轮先落到居中回落。
