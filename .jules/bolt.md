...
## 2026-03-12 - [Optimizing TableOfContents with React.memo]
**Learning:** Components receiving large strings (like raw markdown) or complex props can trigger expensive re-renders even if the props haven't actually changed, particularly if their parent updates frequently (e.g. from global state or layout changes). Recreating inline functions like `scrollToHeading` within an O(n)  loop also causes child DOM elements to re-render unnecessarily.
**Action:** Always wrap heavy display components like `TableOfContents` in `React.memo`, and stabilize callback functions passed to lists using `useCallback`.
