...
## 2026-03-22 - Unrestricted File Upload in Media Versioning
**Vulnerability:** The `createVersion` method in `VersionServiceImpl` accepted new file uploads without validating their extension or magic bytes, allowing an attacker to upload an arbitrary executable file (like `.jsp` or `.php`) as a new version of an existing, safe media file (like `.jpg`).
**Learning:** Security validations applied during the initial creation of an entity (e.g., uploading a new media file) are often overlooked during subsequent update or versioning operations. Both creation and modification endpoints must enforce identical security constraints.
**Prevention:** To prevent arbitrary file upload vulnerabilities (type spoofing) in media services (e.g., `VersionServiceImpl`), explicitly validate that the file extension of any newly uploaded version strictly matches the file extension of the original media file.
