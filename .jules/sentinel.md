...
## 2026-03-25 - Arbitrary File Upload (Type Spoofing) in File Versioning
...
## 2026-03-27 - Missing Rate Limiting on Media Uploads
**Vulnerability:** The `/v1/admin/media/upload` and `/v1/admin/media/upload/batch` endpoints lacked rate limiting, making them susceptible to automated spam, denial-of-service, and storage exhaustion attacks from authenticated users attempting to upload excessive files.
**Learning:** Even endpoints restricted to authenticated users or admin contexts (like media uploads) can be abused and require rate limiting to prevent application logic abuse, storage exhaustion, or denial-of-service conditions.
**Prevention:** Added `@RateLimit(key = "media:upload", count = 30, time = 60, limitType = RateLimit.LimitType.USER)` and `@RateLimit(key = "media:upload:batch", count = 10, time = 60, limitType = RateLimit.LimitType.USER)` to the respective methods in `MediaController`, and added regression tests to `RateLimitVerificationTest`.
