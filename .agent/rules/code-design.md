...
## [v2.2.1] - 2026-02-27
### Changed
- §3.1: 修正 Blog 目录结构描述（`app/` 根级）并同步 alias 映射（`@/* -> ./*`）

### Fixed
- §8.1: 修复文章详情页深层相对导入，统一为 alias 导入规范
...
## 🧾 2026-02-08 文档同步记录 (v2.0.1)

### Added
- AI 服务 Python 3.9 启动兼容策略：`eval_type_backport` 依赖探测 + 本地兜底模块。
- 管理端 AI 图标规则：AI 品牌图标统一 `@lobehub/icons`，通用图标使用 `lucide-react`。

### Changed
- §9.5 运行维护补充：`start.sh` 在 AI 启动前进行依赖完整性检查与自动补装。
- §5.1 管理端实现约束补充：优先使用 CSS 变量 token（`--bg-*`, `--text-*`, `--border-*`）。

### Fixed
- 修复 Python 3.9 环境下 `str | None` 注解求值导致 AI 服务健康检查失败的问题。

### PATCH-2026-02-08
📝 文档更新补丁 PATCH-2026-02-08
【目标章节】§9.5 运行与维护
【原内容】仅描述脚本启停流程，未覆盖 AI 注解兼容问题。
【新内容】补充 AI 启动前依赖探测、缺失依赖自动补装与离线兜底策略。
【变更原因】解决实际运行中 AI 健康检查失败。

📝 文档更新补丁 PATCH-2026-02-08
【目标章节】§5.1 AI Service 架构（管理端）
【原内容】未明确图标来源与主题 token 使用约束。
【新内容】补充图标合规与主题变量优先规则。
【变更原因】确保实现与设计规范一致。

---

## 🧾 2026-02-27 文档同步记录 (v2.0.2)

### Added
- 路线图补充维护任务 `DOC-MAINT-005`，覆盖博客前台导入路径规范化与文档回写闭环。

### Changed
- Blog 前台路径规范从深层相对导入改为 alias 导入：`@/app/*`。
- `apps/blog/tsconfig.json` alias 映射修正为 `@/* -> ./*` 以匹配当前目录结构。

### Fixed
- 修复 `apps/blog/app/posts/(article)/[slug]/page.tsx` 使用 `../../../` 的路径规范偏差。
- 修正文档中 `apps/blog` 结构描述与仓库现状不一致的问题（`app/` 实际在根目录）。

### PATCH-2026-02-27
📝 文档更新补丁 PATCH-2026-02-27
【目标章节】§3.1 前端架构设计
【原内容】`apps/blog` 结构默认按 `src/app` 描述，未与仓库实际目录同步。
【新内容】改为根级 `app/` 结构描述，并补充 `@/* -> ./*` alias 约束。
【变更原因】保证设计文档与仓库结构、导入规范一致，避免实现偏差。

📝 文档更新补丁 PATCH-2026-02-27
【目标章节】§8.1 博客前台模块
【原内容】未明确文章详情页导入路径层级约束。
【新内容】补充“禁止 `../../../` 深层相对导入，统一使用 alias 导入”的实现规则。
【变更原因】提升模块可维护性并降低路径重构风险。
