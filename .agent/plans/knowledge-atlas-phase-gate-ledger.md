...
| P0-02 subpage navigation | Implemented | Dashboard quick links plus Sidebar links for `/atlas`, `/atlas/search`, `/atlas/kps`, `/atlas/graph`, `/atlas/suggestions`; `/atlas/kps` and `/atlas/search` routes added | Reader still enters through Notes or direct note route until note detail has an "Open in Atlas Reader" action |
