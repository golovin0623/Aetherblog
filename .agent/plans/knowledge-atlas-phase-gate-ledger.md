...
| --- | --- | --- | --- |
| P0-01 real Atlas dashboard | Implemented with authenticated smoke evidence | `/atlas` now loads health, KP, pending suggestions, live graph health metrics, recent KP, and quick links; `run-release-smoke.mjs` reached `/admin/atlas` through gateway and matched `Aether Atlas` with warm LCP `508ms` | Re-run on production deployment before release |
| P0-02 subpage navigation | Implemented with authenticated smoke evidence | Dashboard quick links plus Sidebar links for `/atlas`, `/atlas/search`, `/atlas/kps`, `/atlas/graph`, `/atlas/suggestions`; `/atlas/kps` and `/atlas/search` routes added; Note edit header has an Atlas action that saves dirty content before opening `/atlas/reader/note/:noteId`; release smoke opened `/admin/atlas/reader/note/13`, `/admin/atlas/kps`, `/admin/atlas/kp/28`, `/admin/atlas/graph`, and `/admin/atlas/suggestions` | Manual click-path recording can still be added, but route-level gateway smoke now exists |
...
| P0-08 baseline tests/redline evidence | Implemented for Sprint 0 scope with current admin smoke | `go test ./internal/knowledge/...`; `go test ./...`; `pnpm --filter @aetherblog/admin typecheck`; `pnpm --filter @aetherblog/admin build`; Atlas native select/spinner/demo redline grep is clean; R1/R2/R3/R4/R5 gate scripts now provide repeatable command evidence; authenticated gateway smoke report passed 12/12 checks on 2026-05-31 | Live production/user dataset metrics and non-admin browser sessions remain separate release evidence |
...
| R4-01 performance budget gate | Passed current build/runtime budget | `scripts/atlas/performance-budget-gate.mjs --lcp-ms 508 --graph-fps 60.8` passes with Atlas JS `242.9KiB`, CSS `0B`, largest JS `1.38MiB`, LCP `508ms`, and graph FPS `60.8`; the smoke runner records warm-route LCP after the dev gateway has compiled the Atlas dashboard chunk | Re-run against production deployment assets before release |
| R5-01 release smoke report gate | Passed current authenticated gateway smoke | `scripts/atlas/run-release-smoke.mjs` seeds a note, Markdown carrier, annotation, KP pair, relation, suggestion, uploaded PDF, and PDF carrier, then visits Admin, Atlas, Reader note/PDF, KP list/detail, Graph, Suggestions, Notes, KnowledgeBase, AetherHub, and Blog through `http://localhost:7899`; `release-smoke-gate --input output/playwright/atlas-release-smoke-report.json` passed 12/12 checks | Non-admin/multi-user gateway smoke remains separate |
...
| `node --check scripts/atlas/run-release-smoke.mjs` | Passed after adding the authenticated gateway smoke runner |
| `node --check scripts/atlas/release-smoke-gate.mjs` | Passed after aligning the smoke matrix with current admin routes |
| `ATLAS_SMOKE_PASSWORD=... ATLAS_SMOKE_REPORT=output/playwright/atlas-release-smoke-report.json node scripts/atlas/run-release-smoke.mjs` | Passed; seeded note `13`, Markdown carrier `37`, annotation `13`, KP `28`, related KP `29`, relation `10`, suggestion `16`, PDF media `31`, PDF carrier `38`; smoke passed 12/12 with LCP `508ms` and graph FPS `60.8` |
| `node scripts/atlas/release-smoke-gate.mjs --input output/playwright/atlas-release-smoke-report.json` | Passed; required `12`, passed checks `12`, blocked `0`, failed `0`, missing `0` |
| `node scripts/atlas/performance-budget-gate.mjs --lcp-ms 508 --graph-fps 60.8` | Passed current R4 runtime/build budget; Atlas JS `242.9KiB`, largest JS asset `1.38MiB`, LCP `508ms`, graph FPS `60.8` |
...
| --- | --- |
| Phase 1 R1 anchoring recall | Markdown and PDF text-layer version-migration corpora now have repeatable >=90% evidence, PDF media ingest has an internal pypdf extraction + persisted rootText path, and the authenticated gateway smoke covers generated PDF upload -> PDF carrier -> text layer -> PDF Reader; broader real-document PDF corpus and non-admin Reader sessions are still not captured |
| Sprint 1 all-green | P1-01/P1-02/P1-05/P1-06/P1-07/P1-09/P1-10/P1-12/G1-01 now have implementation baselines and admin gateway smoke, but restore/delete flows and non-admin browser paths are still not separately covered |
