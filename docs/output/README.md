...
> **基线:** 2026-05-09 / branch `claude/sad-gould-35d3a6` / migrations 至 000046 / Aether Codex Round 5  
> **沉淀规模:** 10 个一级模块 / 76 份 markdown / ~27,141 行  
...
| 04 | [AI / 搜索 / 系统](./04-backend-ai-search-system/) | AI 网关 / Agent / 关键词+语义 RRF 搜索 / 系统监控 / 中间件 | 8 文 / 2405 行 |

### 前端(`apps/blog/` + `apps/admin/`)

| # | 模块 | 关注面 | 文档量 |
| --- | --- | --- | --- |
| 05 | [博客 · Next.js 15](./05-frontend-blog/) | App Router / RSC / Markdown 管线 / Codex 落地 / iOS WKWebView | 8 文 / 2499 行 |
| 06 | [后台 · Vite + React 19](./06-frontend-admin/) | Shell / 编辑器 / 媒体库 / AI 工具 / 4 套独立 SSE | 10 文 / 3823 行 |

### AI 服务(Python · FastAPI + LiteLLM · `apps/ai-service/`)

| # | 模块 | 关注面 | 文档量 |
| --- | --- | --- | --- |
| 07 | [AI 服务](./07-ai-service-python/) | 单点 LiteLLM 接入 / 模型路由 4 层 / SSE think-block / 26 测试 | 7 文 / 2616 行 |

### 数据 / 设计 / 基础设施

| # | 模块 | 关注面 | 文档量 |
| --- | --- | --- | --- |
| 08 | [数据库 · 46 migration](./08-database-migrations/) | PG17 + pgvector / tsvector / partial HNSW / 蓝绿协议 / dirty self-heal | 6 文 / 2513 行 |
...
```
   ...
   │ RSC + ISR│    │ Zustand    │    │ 26 handler  │    │ LiteLLM      │
   └──────────┘    │ Tanstack-Q │    │ 22 service  │    │ 19+ endpoint │
                   └────────────┘    │ 21 repo     │    │ 26 tests     │
                                     └────┬────────┘    └──────┬───────┘
                                          │                    │
        ┌─────────────────────────────────┴────────────────────┴────────┐
        │  PostgreSQL 17 (pgvector / tsvector / 46 migrations)          │
...
| P1 | 05-blog | `>` / `/` 搜索前缀 UI 已铺,backend 未配套 | tag/command 模式与 default 等价的"假按钮" |
...
4. [10/03-startup-scripts.md](./10-infrastructure-devops/03-startup-scripts.md) 学会 `./start.sh --gateway` 启动
...
- 搜索结果异常 → [04/03-search.md](./04-backend-ai-search-system/03-search.md) §Diagnostics + [03/04-search-and-discovery.md] (前端发现层)
