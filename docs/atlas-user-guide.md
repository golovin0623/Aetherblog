...
- `导入文件` uploads an Obsidian-style Markdown, Readwise CSV, or Zotero RIS file into the current user's Atlas graph.
...
- Zotero `.ris` exports become imported source KPs. Atlas reads common RIS fields such as `TY`, `TI`/`T1`, `AU`/`A1`, `PY`/`Y1`, `DO`, `UR`, `AB`/`N2`, and `KW`, generates a Markdown source note, links each reference to a source annotation, and keeps bibliographic metadata in the KP body.
...
The current A3-05 baseline covers scoped JSON / GraphML / Markdown export, Obsidian-style Markdown import, Readwise CSV import, and Zotero RIS import. Tana, richer Obsidian frontmatter/alias handling, and other external-system adapters remain later ecosystem work.
