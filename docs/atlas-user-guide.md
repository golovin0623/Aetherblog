...
- From a Markdown or PDF Reader header, click `全文 AI 建议`. Atlas sends a bounded slice of the current carrier root text through the AI service and creates carrier-bound pending KP suggestions in the Inbox.
...
Carrier-level extraction is currently a synchronous bounded baseline for Reader usage. Large async batch jobs, progress reporting, per-run cost preview, and budget threshold warnings remain later P2-02/P2-08 work.
