...
**当前选项**: **复活而非废弃**。表（`migration 000054`）+ admin UI 占位（`CreateNotePage.tsx` 的 "AI 索引状态" PanelGroup）均已就绪，仅 worker 缺失。
...
- Markdown Carrier 适配器在 Phase 1 可以**直接接入此表**，避免新建 `carrier_embeddings` 一张表导致两份索引数据。
