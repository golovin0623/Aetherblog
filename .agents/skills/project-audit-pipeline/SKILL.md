...
- **不要超过 10 个并行**(Codex 默认限制)— 模块多就分两波
...
| **P1** | 影响重大功能可用性;违反项目铁律(AGENTS.md);命名/规范偏差导致后续维护困难 |
...
- ❌ **子代理之间对话** — Codex 不支持代理间通信;只能通过主代理串联。
...
不同代理对同一概念可能用不同词(鉴权/认证/auth/login;文章/帖子/post/article)。在 Phase 1 主代理读 AGENTS.md / README 时萃取一份 5-15 行的术语表,放入 Context Pack。**直接消除文档术语漂移。**
...
1. **Phase 0**(2 分钟) — 读 AGENTS.md / README,加载 TodoWrite。
