...
| `chat_handler` | `/v1/chat/*` | 团队聊天（任意已登录用户，authMW + pwdRotated，写路径 120/min/user）：`GET /ws`(WebSocket，复用同源 Cookie 鉴权，承载 message/typing/read/presence)、`GET /conversations`(列表带未读+最后消息)、`POST /conversations/direct`(开私聊)、`POST /conversations/team/:teamId`(开团队群)、`GET /conversations/:id/messages`(历史游标分页)、`POST /conversations/:id/messages`(发送兜底)、`POST /conversations/:id/read`(已读)、`GET /conversations/:id/members`、`POST /attachments`(附件上传,复用 media)、`GET/PUT /settings`(皮肤偏好)。实时层 `internal/realtime`(Hub + Redis Pub/Sub `chat:fanout`)。**预留** sender_type/member_role=`AGENT` 供 Agents 落座。 |
