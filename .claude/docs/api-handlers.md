...
| `agent_handler` | `/v1/agent/*` | 4 路由（`POST chat` SSE、`GET models`、`GET articles`、`GET tags`），任意已登录用户可访问；写 `ai.agent_chat` 审计。**chat SSE 协议**：事件白名单 `retrieval / think / delta / tool_call / tool_result / sources / usage / done / error`；成功路径在 `done` 前必发一条 `usage`（`{promptTokens, completionTokens, totalTokens, estimated}`，provider 经 `stream_options.include_usage` 返回真实用量时 `estimated:false`，否则本地估算 `true`；error / 客户端取消不发；工具循环下 usage 与计费聚合覆盖全部轮次累加，provider 缺真值时 prompt 侧估算按「每轮调用前的完整上下文」逐轮累加，不是只按最终上下文估一次）。**工具调用（function calling）**：请求新增 `enableTools: bool = false`，显式开启且模型 `abilities.functionCall` 为 true 才生效（能力缺失/查询失败静默降级为无工具，不报错）；工具清单为服务端白名单（不接受客户端自定义工具）——`search_knowledge_base(query)`（仅在本次请求已授权 kbIds 范围内语义召回，kbIds 为空时不注册）与 `search_posts(query, limit=5)`（ILIKE 检索已发布/未删除/未隐藏/无密码文章）；模型请求工具时发 `tool_call`（`{type,id,name,arguments}`，arguments 为流式分片拼装完成的 JSON 字符串）→ 服务端执行（pydantic 校验 query ≤500、SQL 全参数化、单工具 10s 超时、异常折叠）→ 发 `tool_result`（`{type,id,name,result,isError}`，result 截断 ≤2000 字符）→ assistant(tool_calls)+tool 消息回喂上下文继续流式；最多 4 轮工具循环，超限撤 tools 并注入 system 提示强制直接作答。防护硬限：单个调用 arguments 分片累加 >8KB（8192 字节）即标记 oversized —— 停止累加、不执行，SSE 事件与上下文回填一律用「截断到 8KB + 『…（参数超长已截断）』」版本并回 `isError` 回执「工具参数超长，已拒绝执行」（防超长 SSE 行撑爆 Go scanner）；单轮 >8 个调用时第 9 个起不逐个下发/回填，合并为一条 `isError` 回执「本轮工具调用超过上限，已忽略 N 个」（协议上保留第 9 个调用进 assistant tool_calls 作回执挂载点，其余彻底忽略）。mock_mode 下 enableTools 请求返回固定 tool_call → tool_result → delta 联调序列。**图片输入（vision）**：`messages[].content` 接受纯字符串或 OpenAI content-parts 数组（`{type:"text",text}` / `{type:"image_url",image_url:{url}}`）；仅接受 `data:image/(png|jpeg|webp|gif);base64` 内联 Data URL（防 SSRF），单图解码后 ≤5MB、单条消息 ≤4 图、每请求 ≤8 图，文本硬限（单条 8K / 总 32K）只计文本片段；所选模型 `abilities.vision` 不为 true 时 400。body 上限 24MB（`agentChatBodyLimit`）。**models**：`AgentModelItem` 含 `inputCostPer1M / outputCostPer1M`（USD/1M tokens；per_1k 列 ×1000，缺失回退 `capabilities.pricing`，均无则 null） |
...
| `agent_session_handler` | `/v1/agent/sessions/*`（migration 000089） | 灵境会话云同步（跨设备漫游，任意已登录用户，越权/不存在一律 404）：`GET /sessions`（列表，不含 messages、含 `messageCount`，置顶优先按更新倒序，`?limit=` 默认 100 上限 500）、`GET /sessions/:id`（详情含全量 messages 按 seq 升序）、`PUT /sessions/:id`（**整会话 upsert**：body = meta + 全量 messages，事务内全量替换，幂等可重放；**LWW**：库内 `client_updated_at` > 请求 `updatedAt` → HTTP 409 + `data`=服务端完整版本，相等视为重放接受）、`DELETE /sessions/:id`。会话/消息 id 客户端生成（`^[A-Za-z0-9_-]{8,64}$`，非法 400）；消息可选元数据（think/sources/retrieval/usage/attachments 元信息(不含 dataUrl)/translation/requestSnapshot/error 等）整包进 `payload` JSON 透传。**配额与长度硬限（超限 400 中文文案）**：单用户会话数 ≤500（仅新建路径校验，配额满时更新已有会话仍放行）、单会话消息 ≤2000 条、单条消息 content ≤64K 字符、draft ≤16K 字符、title >300 rune 截断。写路径（PUT/DELETE）`rate:agent:sessions` 60/min/user（onlyMutating），body 上限 4MB |
...
| `stats_handler` | `/v1/admin/stats/*` | 7 路由：dashboard、top-posts、visitor-trend、archives、ai-dashboard、ai-pricing-gaps、ai-cost-archive。**ai-dashboard `taskDistribution[]`** 每项除 `task/calls/percentage/tokens/cost` 外，含 `tokensIn/tokensOut/avgLatencyMs` 与今日（服务器时区 `CURRENT_DATE`）子集 `todayCalls/todayTokensIn/todayTokensOut/todayCost/todayAvgLatencyMs`（供 admin 数据分析页「灵境对话」卡片按 `taskType=agent_chat` 消费，token 为 provider 真值优先、估算兜底） |
