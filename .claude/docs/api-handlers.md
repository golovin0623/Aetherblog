...
| `agent_handler` | `/v1/agent/*` | 4 路由（`POST chat` SSE、`GET models`、`GET articles`、`GET tags`），任意已登录用户可访问；写 `ai.agent_chat` 审计。**chat SSE 协议**：事件白名单 `retrieval / think / delta / sources / usage / done / error`；成功路径在 `done` 前必发一条 `usage`（`{promptTokens, completionTokens, totalTokens, estimated}`，provider 经 `stream_options.include_usage` 返回真实用量时 `estimated:false`，否则本地估算 `true`；error / 客户端取消不发）。**图片输入（vision）**：`messages[].content` 接受纯字符串或 OpenAI content-parts 数组（`{type:"text",text}` / `{type:"image_url",image_url:{url}}`）；仅接受 `data:image/(png|jpeg|webp|gif);base64` 内联 Data URL（防 SSRF），单图解码后 ≤5MB、单条消息 ≤4 图、每请求 ≤8 图，文本硬限（单条 8K / 总 32K）只计文本片段；所选模型 `abilities.vision` 不为 true 时 400。body 上限 24MB（`agentChatBodyLimit`）。**models**：`AgentModelItem` 含 `inputCostPer1M / outputCostPer1M`（USD/1M tokens；per_1k 列 ×1000，缺失回退 `capabilities.pricing`，均无则 null） |
