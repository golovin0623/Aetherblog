...
## 2026-06-08 - Segmented Control Keyboard Accessibility
**Learning:** Segmented controls implemented using Next.js `<Link>` components (such as the View Mode toggles in `MobileNavSwitch.tsx` and `BlogHeader.tsx`) often lack proper focus indicators, making them completely invisible to keyboard users. Furthermore, since these controls use rounded caps, standard rectangular focus rings (`focus-visible:ring-2`) can look awkward or overlap with the pill-shaped background.
**Action:** When implementing segmented controls using `<Link>` elements, wrap them in a `role="group"` container. Use `aria-current="page"` (with `undefined` for inactive) to indicate the active segment, and ensure comprehensive utility classes (such as `focus-visible:ring-offset-[var(--bg-secondary)] focus-visible:rounded-full`) are applied to maintain rounded borders and match background offsets for proper keyboard accessibility.
