...
type SummaryRequest struct {
	...
	Style         string  `json:"style,omitempty"`                     // 摘要风格（可选，如 formal/casual）
...
type TitlesRequest struct {
	...
	Count         int    `json:"count,omitempty"`                      // 希望生成的标题数量（可选）
	Style         string `json:"style,omitempty"`                      // 标题风格（可选）
	Model         string `json:"model,omitempty"`                      // 指定使用的模型名称（可选）
	PromptVersion string `json:"promptVersion,omitempty"`              // 提示词版本号（可选）
	ModelID       string `json:"modelId,omitempty"`                    // 模型 ID（可选）
	ProviderCode  string `json:"providerCode,omitempty"`               // AI 服务提供商代码（可选）
...
type PolishRequest struct {
	...
	PolishType    string `json:"polishType,omitempty"`                 // 润色类型（可选，如 grammar/style/concise）
	Style         string `json:"style,omitempty"`                      // 目标文章风格（可选）
	Model         string `json:"model,omitempty"`                      // 指定使用的模型名称（可选）
	PromptVersion string `json:"promptVersion,omitempty"`              // 提示词版本号（可选）
	ModelID       string `json:"modelId,omitempty"`                    // 模型 ID（可选）
	ProviderCode  string `json:"providerCode,omitempty"`               // AI 服务提供商代码（可选）
...
type OutlineRequest struct {
	Topic           string `json:"topic" validate:"required"`           // 文章主题（必填）
