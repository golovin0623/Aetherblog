...
// AdminReplyCommentRequest 是管理端回复评论接口的请求体 DTO。
type AdminReplyCommentRequest struct {
	Content string `json:"content" validate:"required,min=1,max=5000"` // 管理员回复内容
}
...
type CommentFilter struct {
	...
	Keyword  string // 关键词搜索（匹配评论内容、昵称、邮箱或文章标题）
