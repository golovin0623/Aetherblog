...
type RegisterRequest struct {
	...
	Password string `json:"password" validate:"required,min=8,max=128,password_complexity"` // 密码，长度 8~128 个字符，需含大小写字母和数字（必填）
...
type ChangePasswordRequest struct {
	...
	NewPassword     string `json:"newPassword"     validate:"required,min=8,max=128,password_complexity"` // 新密码，长度 8~128 个字符，需含大小写字母和数字（必填）
}
