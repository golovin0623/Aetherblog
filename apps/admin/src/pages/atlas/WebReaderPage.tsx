...
// 稳定锚定空间。这里不做远程抓取，只读取已保存的快照并支持标注/AI 建议。
...
const WEB_TEXT_LAYER_CONFORMS_TO = 'https://aetherblog.local/atlas/web-text-layer';

const initial: WebReaderState = {
  carrier: null,
  layer: null,
  annotations: [],
  loading: true,
...
export default function WebReaderPage() {
  ...
  const [state, setState] = useState<WebReaderState>(initial);
  ...
  useEffect(() => {
    if (!carrierId) {
      setState({ ...initial, loading: false, error: '无效的 Web carrier ID' });
      return;
    }
    let cancelled = false;
    void (async () => {
      try {
        ...
        if (cancelled) return;
        if (carrierRes.data.type !== 'web') {
          setState({ ...initial, loading: false, error: '该 carrier 不是 Web 快照' });
          return;
        }
        setState({
        ...
        if (cancelled) return;
        setState({ ...initial, loading: false, error: extractApiErrorMessage(err, '加载 Web 快照失败') });
      }
    })();
    return () => {
      cancelled = true;
  ...
  const sourceUrl = useMemo(() => {
    ...
    const metaUrl = typeof state.carrier.metadata?.sourceUrl === 'string' ? state.carrier.metadata.sourceUrl : '';
  ...
  const handleHighlight = useCallback(async () => {
    ...
    if (!root || !selection || selection.rangeCount === 0 || selection.isCollapsed) {
      toast.message('请先选中要标注的 Web 正文');
      return;
    }
    const range = selection.getRangeAt(0);
    if (!root.contains(range.commonAncestorContainer)) {
      toast.message('请在 Web 正文区域内选择文本');
      return;
    }
    ...
    const fragment: FragmentSelector = {
      ...
      conformsTo: WEB_TEXT_LAYER_CONFORMS_TO,
    ...
    try {
      const res = await atlasService.createAnnotation({
        ...
        bodyMeta: { source: 'web-text-layer', sourceUrl },
        anchorState: 'anchored',
        anchorScore: 1,
      });
      setState((s) => ({ ...s, annotations: [res.data, ...s.annotations] }));
      selection.removeAllRanges();
      toast.success('Web 标注已创建');
    } catch (err) {
      toast.error(extractApiErrorMessage(err, '创建 Web 标注失败'));
    }
  }, [sourceUrl, state.carrier, state.layer]);
  ...
  const handleGenerateCarrierSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const count = res.data?.length ?? 0;
      toast.success(count > 0 ? `已从 Web 正文生成 ${count} 条 AI 建议，前往 Inbox 处理` : 'AI 未生成可用建议');
    } catch (err) {
      toast.error(extractApiErrorMessage(err, '生成 Web AI 建议失败'));
    } finally {
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.18em] text-[var(--ink-muted)]">
              Web · carrier #{state.carrier.id} · {state.layer.charCount} chars
            </p>
      ...
      <main className="flex min-h-0 flex-1 overflow-hidden">
        <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--bg-substrate)] px-5 py-5">
          <article className="mx-auto max-w-5xl overflow-hidden rounded-lg border border-[color-mix(in_oklch,var(--ink-primary)_8%,transparent)] bg-[var(--bg-leaf)] shadow-sm">
            <header className="flex items-center justify-between gap-3 border-b border-[color-mix(in_oklch,var(--ink-primary)_6%,transparent)] px-4 py-2">
              <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--ink-muted)]">Web text layer</span>
              <span className="font-mono text-[10px] text-[var(--ink-muted)]">{state.layer.storageUri}</span>
            </header>
            <pre
              ref={textRef}
              data-atlas-web-reader
        ...
        <aside className="w-[360px] shrink-0 overflow-y-auto border-l border-[color-mix(in_oklch,var(--ink-primary)_8%,transparent)] bg-[var(--bg-leaf)] px-4 py-4">
          <h2 className="mb-3 text-xs uppercase tracking-[0.18em] text-[var(--ink-muted)]">
            Web 标注 · {state.annotations.length}
          </h2>
          {state.annotations.length === 0 ? (
            <p className="text-xs text-[var(--ink-secondary)]">暂无 Web 标注</p>
          ) : (
