...
// 为稳定 text layer。这里读取该文本层并支持标注/AI 建议。
...
const BLOG_POST_TEXT_LAYER_CONFORMS_TO = 'https://aetherblog.local/atlas/blog-post-text-layer';

const initial: BlogPostReaderState = {
  carrier: null,
  layer: null,
  annotations: [],
  loading: true,
...
function postIDFromSource(sourceUri: string): number | null {
  const match = sourceUri.trim().match(/^posts:\/\/(\d+)$/);
...
export default function BlogPostReaderPage() {
  ...
  const [state, setState] = useState<BlogPostReaderState>(initial);
  ...
  useEffect(() => {
    if (!carrierId) {
      setState({ ...initial, loading: false, error: '无效的文章 carrier ID' });
      return;
    }
    let cancelled = false;
    void (async () => {
      try {
        ...
        if (cancelled) return;
        if (carrierRes.data.type !== 'blog_post') {
          setState({ ...initial, loading: false, error: '该 carrier 不是文章快照' });
          return;
        }
        setState({
        ...
        if (cancelled) return;
        setState({ ...initial, loading: false, error: extractApiErrorMessage(err, '加载文章快照失败') });
      }
    })();
    return () => {
      cancelled = true;
  ...
  const postId = useMemo(() => (state.carrier ? postIDFromSource(state.carrier.sourceUri) : null), [state.carrier]);
  ...
  const handleHighlight = useCallback(async () => {
    ...
    if (!root || !selection || selection.rangeCount === 0 || selection.isCollapsed) {
      toast.message('请先选中要标注的文章正文');
      return;
    }
    const range = selection.getRangeAt(0);
    if (!root.contains(range.commonAncestorContainer)) {
      toast.message('请在文章正文区域内选择文本');
      return;
    }
    ...
    const fragment: FragmentSelector = {
      ...
      conformsTo: BLOG_POST_TEXT_LAYER_CONFORMS_TO,
    ...
    try {
      const res = await atlasService.createAnnotation({
        ...
        bodyMeta: { source: 'blog-post-text-layer', postId },
        anchorState: 'anchored',
        anchorScore: 1,
      });
      setState((s) => ({ ...s, annotations: [res.data, ...s.annotations] }));
      selection.removeAllRanges();
      toast.success('文章标注已创建');
    } catch (err) {
      toast.error(extractApiErrorMessage(err, '创建文章标注失败'));
    }
  }, [postId, state.carrier, state.layer]);
  ...
  const handleGenerateCarrierSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const count = res.data?.length ?? 0;
      toast.success(count > 0 ? `已从文章正文生成 ${count} 条 AI 建议，前往 Inbox 处理` : 'AI 未生成可用建议');
    } catch (err) {
      toast.error(extractApiErrorMessage(err, '生成文章 AI 建议失败'));
    } finally {
      ...
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-[color-mix(in_oklch,var(--ink-primary)_8%,transparent)] bg-[var(--bg-leaf)] px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.18em] text-[var(--ink-muted)]">
              Post · carrier #{state.carrier.id} · {state.layer.charCount} chars
            </p>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {postId ? (
            ...
            >
              <ExternalLink className="h-4 w-4" /> 原文章
            </Link>
          ) : null}
          <button
            type="button"
      ...
      <main className="flex min-h-0 flex-1 overflow-hidden">
        <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--bg-substrate)] px-5 py-5">
          <article className="mx-auto max-w-5xl overflow-hidden rounded-lg border border-[color-mix(in_oklch,var(--ink-primary)_8%,transparent)] bg-[var(--bg-leaf)] shadow-sm">
            <header className="flex items-center justify-between gap-3 border-b border-[color-mix(in_oklch,var(--ink-primary)_6%,transparent)] px-4 py-2">
              <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--ink-muted)]">Blog post text layer</span>
              <span className="font-mono text-[10px] text-[var(--ink-muted)]">{state.layer.storageUri}</span>
            </header>
            <pre
              ref={textRef}
              data-atlas-blog-post-reader
        ...
        <aside className="w-[360px] shrink-0 overflow-y-auto border-l border-[color-mix(in_oklch,var(--ink-primary)_8%,transparent)] bg-[var(--bg-leaf)] px-4 py-4">
          <h2 className="mb-3 text-xs uppercase tracking-[0.18em] text-[var(--ink-muted)]">
            文章标注 · {state.annotations.length}
          </h2>
          {state.annotations.length === 0 ? (
            <p className="text-xs text-[var(--ink-secondary)]">暂无文章标注</p>
          ) : (
